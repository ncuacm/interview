...
+ Mysql的几种索引
  ...
  + 应用层次来区分有普通索引，唯一索引，复合索引
    ...
    + 非聚簇索引：叶子节点还是索引节点，不过存储了对应的数据上的指针。非聚集索引不要求连续，他只是在逻辑地址上面的连续，不要求物理地址的连续性。非聚簇索引表示的是主键之外的属性当作索引（叫做辅助索引），和聚簇索引不同的是叶子节点存储的不是数据，而是对应的这一行数据的主键，当我们找到这个主键的时候，我们就需要通过回表的操作从聚簇索引里面找到这条数据。但是其实非聚簇索引不一定需要进行回表的操作，原因是这里涉及到一个索引覆盖的问题。如果我们需要查找的数据恰好可以在辅助索引上面可以完全找到我们本次查询需要的数据，那么就不需要执行回表的操作了。
  ...
  + 为什么树的高度会影响磁盘的IO的次数呢？
    + 我们都知道，InnoDB的存储引擎存放数据的基本单位是页，一个页的大小大概是16KB。InnoDB在同一个页存储同一高度的树的节点的数据，所以，如果树的高度越高，那么系统进行一次磁盘IO的次数也就会越多。
  + Mysql的前缀索引？
    + 前缀索引是指对文本或者字符串的前几个字符建立索引，这样索引的长度更短，同时查询的速度更快。使用的场景一般是前缀区分度高的场景。
  + Mysql的最左前缀原则
...
+ 索引的优化
  + 需要联系实际的业务场景，将区分度高的索引放到前面，同时少用select *.
...
+ Mysql的WAL技术
  + Writing Ahead Logging，预写入日志，采用的是先写日志，然后再写磁盘的方式，因为每次写磁盘消耗的时间都是非常久的。
